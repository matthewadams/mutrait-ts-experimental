...
StartTest((t) => {
  ...
  t.it('should work', (t) => {
    ...
    class BlogPost extends Taggable.mix(Nameable.mix(Object)) {
      ...
      protected _testSetTag(value: string) {
        if (!value) throw new Error('no tag given')
        return value
      }

      protected _testSetName(value: string) {
        if (!value) throw new Error('no name given')
        return value
      }
    }
    ...
    t.is(bp.name, name)
    t.is(bp.tag, tag)
    t.is(bp instanceof Taggable, true)
    t.is(bp instanceof Nameable, true)

    const takesTaggable = (it: ITaggable) => {
      return it.tag
    }
    const takesNameable = (it: INameable) => {
      return it.name
    }

    t.is(takesNameable(bp), name)
    t.is(takesTaggable(bp), tag)
  })
})
