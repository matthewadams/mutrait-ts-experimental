...
StartTest((t) => {
  ...
  t.it('Should be able to use iterators, derived from the memoized iterator in any order', (t) => {
    ...
    t.isDeeply([iterator2.next().value, iterator3.next().value], [1, '11'])

    // opposite order
    t.isDeeply([iterator3.next().value, iterator2.next().value], ['1212', 2])

    t.isDeeply([iterator2.next().value, iterator3.next().value], [3, '123123'])

    t.isDeeply([iterator3.next().done, iterator2.next().done], [true, true])
  })
})
